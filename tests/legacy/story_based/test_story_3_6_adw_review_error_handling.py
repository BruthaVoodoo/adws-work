#!/usr/bin/env python3
"""
Unit tests for Story 3.6: Update error handling in adw_review.py for OpenCode

Tests migration from Copilot CLI check to OpenCode server availability check,
verifying that adw_review.py now uses check_opencode_server_available()
...
from scripts.adw_review import check_env_vars


class TestStory36AdwReviewErrorHandling:
    """Test suite for Story 3.6: adw_review.py error handling migration to OpenCode."""
    ...
    def test_check_env_vars_opencode_server_available_calls_check_function(
        ...
        with patch(
            "scripts.adw_review.check_opencode_server_available",
            return_value=True,
        ) as mock_check:
    ...
    def test_check_env_vars_migration_verification_no_copilot_which(
        ...
        with patch(
            "scripts.adw_review.check_opencode_server_available", return_value=True
        ):
    ...
    def test_check_env_vars_no_longer_requires_copilot_cli(
        ...
        # This test verifies function works without checking for copilot binary
    ...
    def test_check_opencode_server_available_imported_from_opencode_module(
        ...
        from scripts import adw_review

        # Check that check_opencode_server_available is available
        assert hasattr(adw_review, "check_opencode_server_available")
    ...
    def test_check_env_vars_error_message_references_opencode_not_copilot(
        ...
        with (
            ...
            # Get error message that was logged
