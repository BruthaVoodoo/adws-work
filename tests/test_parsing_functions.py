...
class TestExtractKeywords:
    ...
    def test_extract_success_keywords(self):
        ...
        output = "✓ Implementation Summary completed successfully"
        keywords = extract_keywords(output)
        
        assert keywords['success_indicators']
        assert '✓' in keywords['success_indicators']
    ...
    def test_extract_error_keywords(self):
        ...
        assert '✗' in keywords['error_indicators']
    ...
    def test_extract_warning_keywords(self):
        ...
        assert '⚠' in keywords['warning_indicators']
    ...
    def test_all_keywords(self):
        ...
        output = "✓ SUCCESS completed ✗ ERROR ⚠ WARNING"
        keywords = extract_keywords(output)
...
- Lines removed: 350"""
    ...
    def test_case_insensitive_metrics(self):
        """Test that metric extraction works with various formats."""
        # Note: Patterns are specific to certain formats, not purely case-insensitive
        output = "5 files changed, 100 lines added"
        files, added, removed = extract_metrics(output)
...
class TestExtractTestResults:
    ...
    def test_pytest_format(self):
        ...
        output = "5 tests passed, 2 failed in 1.23s"
        result = extract_test_results(output)
...
WARNING: Deprecated API used"""
    ...
    def test_parse_with_ansi_codes(self):
        ...
        output = "\x1B[32m✓ Implementation Complete\x1B[0m\n5 files changed"
...
# ============================================================================
# TESTS FOR parse_local_test_output_helper() from adw_test.py
# ============================================================================

# Helper function - parse_local_test_output from adw_test.py
def parse_local_test_output_helper(output: str, exit_code: int, test_command: str = "") -> Tuple[List[TestResult], int, int]:
...
class TestParseLocalTestOutput:
    """Tests for parse_local_test_output from adw_test.py."""

    def test_parse_pytest_passing_tests(self):
        """Test parsing pytest output with passing tests."""
        
        output = """tests/test_file.py::test_one PASSED
tests/test_file.py::test_two PASSED"""
        
        results, passed, failed = parse_local_test_output_helper(output, 0, "pytest")
    ...
    def test_parse_pytest_failing_tests(self):
        """Test parsing pytest output with failing tests."""
        output = """tests/test_file.py::test_one PASSED
tests/test_file.py::test_two FAILED"""
        
        results, passed, failed = parse_local_test_output_helper(output, 1, "pytest")
    ...
    def test_parse_pytest_mixed_results(self):
        """Test parsing pytest with mixed pass/fail/skip."""
        output = """tests/test_file.py::test_one PASSED
tests/test_file.py::test_two FAILED
tests/test_file.py::test_three SKIPPED"""
    ...
    def test_parse_pytest_error_status(self):
        """Test parsing pytest with ERROR status."""
        output = "tests/test_file.py::test_one ERROR"
    ...
    def test_parse_pytest_complex_path(self):
        """Test parsing pytest with complex file paths."""
        output = "src/tests/integration/test_api.py::TestClass::test_method PASSED"
        
        results, passed, failed = parse_local_test_output_helper(output, 0, "pytest")
        
        assert len(results) == 1
        assert "test_api.py" in results[0].test_name

    def test_parse_no_tests_success_exit(self):
        """Test parsing when no tests match but exit code is 0."""
        output = "No tests found"
        
        results, passed, failed = parse_local_test_output_helper(output, 0, "pytest")
        
        # Should create a synthetic test result for the suite
        assert len(results) == 1
        assert results[0].passed is True
        assert "Suite" in results[0].test_name

    def test_parse_no_tests_failure_exit(self):
        """Test parsing when no tests match but exit code is non-zero."""
        output = "Error: tests not found"
        
        results, passed, failed = parse_local_test_output_helper(output, 1, "pytest")
        
        # Should create a synthetic failed test result
        assert len(results) == 1
        assert results[0].passed is False
        assert "exit code" in results[0].error.lower()

    def test_parse_test_with_special_names(self):
        """Test parsing tests with special characters in names."""
        output = """tests/test_file.py::test_with_underscore_123 PASSED
tests/test_file.py::test_CamelCase FAILED"""
    ...
    def test_parse_preserves_command_info(self):
        """Test that parsed results preserve test command info."""
        output = "tests/test.py::test_one PASSED"
        test_cmd = "uv run pytest -v"
        
        results, _, _ = parse_local_test_output_helper(output, 0, test_cmd)
        
        assert results[0].execution_command == test_cmd
...
class TestParsingRobustness:
    """Tests for robustness and failure handling."""

    def test_copilot_parser_with_mixed_line_endings(self):
        """Test parsing with mixed line endings (CRLF and LF)."""
        output = "✓ Success\r\n5 files changed\n100 insertions(+)"
        result = parse_copilot_output(output)
    ...
    def test_metrics_extraction_with_commas(self):
        """Test metric extraction when numbers have commas."""
        output = "1,000 files changed, 10,500 insertions(+)"
        files, added, removed = extract_metrics(output)
        
        # Current implementation may not handle commas perfectly
        # This test documents the limitation
        assert files >= 0  # May be 0 or 1000 depending on regex

    def test_json_parsing_with_trailing_commas(self):
        """Test JSON parsing with non-standard trailing commas."""
    ...
    def test_parse_output_with_null_bytes(self):
        """Test parsing output that may contain null bytes."""
        output = "Test\x00Output\x00Data"
        # Should not crash
        result = parse_copilot_output(output)
        assert result is not None

    def test_concurrent_parsing_independent(self):
        """Test that parsing different outputs independently doesn't interfere."""
        output1 = "✓ Success\n5 files changed"
        output2 = "✗ Failed\n0 files changed"
        
        result1 = parse_copilot_output(output1)
        result2 = parse_copilot_output(output2)
        
        # Each should parse independently
        assert result1.success is True
        assert result2.success is False

    def test_extract_metrics_with_floats(self):
        """Test metric extraction when values are floating point."""
        output = "1.5 files changed, 100.5 insertions"
        files, added, removed = extract_metrics(output)
        
        # Regex looks for \d+ (integers), so floats may not match
        assert files >= 0

    def test_parse_json_with_bom(self):
        """Test JSON parsing with UTF-8 BOM."""
        # BOM character
        json_str = '\xef\xbb\xbf{"key": "value"}'
        # May fail depending on implementation
        try:
            result = parse_json(json_str)
            assert result is not None
        except ValueError:
            # BOM handling may not be supported
            pass

    def test_keyword_extraction_with_empty_list(self):
        """Test extract_keywords handles empty indicators gracefully."""
        keywords = extract_keywords("No indicators here")
        
        assert 'success_indicators' in keywords
        assert 'error_indicators' in keywords
        assert 'warning_indicators' in keywords
