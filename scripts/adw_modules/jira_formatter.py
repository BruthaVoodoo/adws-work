...
def format_implementation_summary(response: AgentPromptResponse, plan_validation=None) -> str:
    ...
    if response.validation_status:
        ...
        status_text = status_icons.get(response.validation_status, response.validation_status)
    ...
    if plan_validation:
        ...
        lines.append(f"- **Steps Executed:** {plan_validation.executed_steps}/{plan_validation.total_steps}")
        
        if plan_validation.missing_steps:
            lines.append(f"- **Missing Steps:** {', '.join(plan_validation.missing_steps)}")
        
        if plan_validation.optional_steps_skipped:
            lines.append(f"- **Optional Steps Skipped:** {', '.join(plan_validation.optional_steps_skipped)}")
...
def format_validation_report(response: AgentPromptResponse, plan_validation=None) -> str:
    ...
    if plan_validation:
        lines.append("### Plan Execution")
        lines.append(f"- Steps: {plan_validation.executed_steps}/{plan_validation.total_steps}")
        if plan_validation.missing_steps:
