...
with GitHub Copilot models (Claude Sonnet 4, Claude Haiku 4.5). This module is
kept for reference and historical purposes only.

Active implementation uses structured OpenCode HTTP API responses with
OpenCodeHTTPClient from scripts/adw_modules/opencode_http_client.py.

The OpenCode API provides structured Part responses (text, tool_use, tool_result,
code_block) that replace the need for parsing unstructured Copilot CLI text output.

See: scripts/adw_modules/opencode_http_client.py for active implementation.
"""
...
def extract_keywords(output: str) -> Dict[str, List[str]]:
    ...
    success_keywords = [
        ...
        "Implementation is complete and working perfectly",
    ]
    ...
    try:
        ...
        if not output or not output.strip():
            ...
            return ParsedCopilotOutput(
                success=False, validation_status="empty", raw_output=output
            )
        ...
        validation_status = determine_validation_status(
            keywords, errors, warnings, clean_output, files_changed
        )
        ...
        logger.debug(
            f"Parsed output: success={result.success}, "
