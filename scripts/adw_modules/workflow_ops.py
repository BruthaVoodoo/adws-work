...
from adw_modules.bitbucket_ops import check_pr_exists, create_pull_request as bb_create_pr, update_pull_request
...
def classify_issue(
    issue: GitHubIssue, adw_id: str, logger: logging.Logger, domain: str = "ADW_Core", workflow_agent_name: Optional[str] = None
...
def build_plan(
    issue, command: str, adw_id: str, logger: logging.Logger, domain: str = "ADW_Core", workflow_agent_name: Optional[str] = None
) -> AgentPromptResponse:
    ...
    logger.debug(f"Build plan request: {request.model_dump_json(indent=2, by_alias=True)}")
    response = execute_template(request)
    logger.debug(f"Build plan response: {response.model_dump_json(indent=2, by_alias=True)}")
...
"""
    ...
    try:
        ...
        logger.error(error_msg)
        return AgentPromptResponse(output=str(e), success=False, validation_status="error")
...
def create_pull_request(
    ...
    try:
        ...
        logger.debug(f"Calling AI agent for PR creation with prompt length: {len(prompt)}")
        ...
        response = execute_template(request)
        logger.debug(f"AI agent response success: {response.success}, length: {len(response.output)}")
        logger.debug(f"AI agent raw response (first 500 chars): {response.output[:500]}")
        ...
        try:
            ...
            logger.debug(f"Parsed PR data type: {type(pr_data).__name__}, keys: {list(pr_data.keys()) if isinstance(pr_data, dict) else 'N/A'}")
...
def ensure_plan_exists(state: ADWState, issue_number: str) -> str:
    ...
    if f"-{issue_number}-" in branch:
        plans = glob.glob(str(config.ai_docs_dir / "specs" / "*" / f"*{issue_number}*.md"))
        if plans:
            return plans[0]
...
def create_or_find_branch(
    ...
    if branch_name:
        ...
        if current != branch_name:
            result = subprocess.run(["git", "checkout", branch_name], capture_output=True, text=T)
            if result.returncode != 0:
