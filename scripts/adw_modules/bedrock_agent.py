...
def invoke_model(prompt: str, model_id: str) -> AgentPromptResponse:
    ...
    if not client:
        return AgentPromptResponse(output="Bedrock client not initialized.", success=False)
    ...
    try:
        ...
        if response_body.get("content") and len(response_body["content"]) > 0:
            ...
            error_output = f"Bedrock model returned no content. Stop reason: {stop_reason}"
