...
# /// script
# dependencies = ["python-dotenv", "pydantic", "requests", "jira", "rich"]
# ///
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ADW Build - AI Developer Workflow for agentic building")
    ...
    if state:
        ...
        jira_make_issue_comment(issue_number, f"{adw_id}_ops: 🔍 Found existing state - resuming build\n```json\n{json.dumps(state.data, indent=2)}\n```")
    else:
    ...
    if not state.get("branch_name"):
        ...
        if rich_console:
            rich_console.error(error_msg)
        jira_make_issue_comment(issue_number, format_issue_message(adw_id, 'ops', f'❌ {error_msg}'))
        sys.exit(1)
    ...
    if result.returncode != 0:
        ...
        if rich_console:
            rich_console.error(error_msg)
        jira_make_issue_comment(issue_number, format_issue_message(adw_id, 'ops', f'❌ Failed to checkout branch {branch_name}'))
        sys.exit(1)
    ...
    jira_make_issue_comment(issue_number, format_issue_message(adw_id, 'ops', '✅ Starting implementation phase'))
    ...
    jira_make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, '✅ Implementing solution'))
    
    if rich_console:
        with rich_console.spinner("Implementing solution using AI agent..."):
    ...
    if not implement_response.success:
        ...
        jira_make_issue_comment(
            issue_number, 
            format_issue_message(adw_id, AGENT_IMPLEMENTOR, f'❌ Error implementing solution. See attached implementation_details.md for full logs.')
        )
        ...
        if rich_console:
            ...
            print(f"2. Commit Changes: git add -A && git commit -m 'feat: Implementation for {issue_number}'")
    ...
    jira_make_issue_comment(
        issue_number, 
        format_issue_message(adw_id, AGENT_IMPLEMENTOR, '✅ Solution implemented. See attached implementation_details.md for execution logs.')
    )
    ...
    if not issue_command:
        ...
        issue_command, error = classify_issue(
            issue, 
            adw_id, 
            logger
        )
        if error:
            ...
            if rich_console:
                rich_console.warning("Defaulting to /feature after classification error")
        else:
    ...
    if error:
        ...
        if rich_console:
            rich_console.error(error_msg)
        jira_make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, f'❌ Error creating commit message: {error}'))
        sys.exit(1)
    ...
    if not success:
        ...
        if rich_console:
            rich_console.error(error_msg)
        jira_make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, f'❌ Error committing implementation: {error}'))
        sys.exit(1)
    
    logger.info(f"Committed implementation: {commit_msg}")
    if rich_console:
        rich_console.success("Implementation committed successfully")
    jira_make_issue_comment(issue_number, format_issue_message(adw_id, AGENT_IMPLEMENTOR, '✅ Implementation committed'))
    ...
    jira_make_issue_comment(issue_number, format_issue_message(adw_id, 'ops', '✅ Implementation phase completed'))
    ...
    try:
        ...
        # Post short comment
        jira_make_issue_comment(issue_number, f"{adw_id}_ops: ✅ Build complete. See attached build_summary.md for details.")
    except Exception as e:
        ...
        jira_make_issue_comment(issue_number, f"{adw_id}_ops: 📋 Final implementation state (Fallback):\n```json\n{json.dumps(state.data, indent=2)}\n```")
